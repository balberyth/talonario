...
function liberar() {
  ...
  if (
    ...
    let clienteIndex = clientes.value.findIndex((cliente) =>
      cliente.boletas.includes(index)
    );
    // Eliminar la boleta de la lista de boletas del cliente
    if (clienteIndex !== -1) {
      clientes.value[clienteIndex].boletas = clientes.value[
        clienteIndex
      ].boletas.filter((boletaIndex) => boletaIndex !== index);
...
function marcarPagado() {
  ...
  const participanteIndex = clientes.value.findIndex((cliente) =>
    cliente.boletas.includes(index)
  );
...
const pdf = () => {
  ...
  const totalPrecioBoletas =
    filteredBoletas.filter((boleta) => boleta.pagar === "si").length *
    precioBoleta.value;
  const totalPrecioFaltanteBoletasReservadas =
    filteredBoletas.filter((boleta) => boleta.pagar === "no").length *
    precioBoleta.value;
  doc.text(
    "Total de boletas compradas: ${totalBoletas}",
    10,
    doc.autoTable.previous.finalY + 10
  );
  doc.text(
    "Total del precio de las boletas compradas: ${formatearNumero(totalPrecioBoletas)}",
    10,
    doc.autoTable.previous.finalY + 20
  );
  doc.text(
    "Total del precio faltante de las boletas reservadas: ${formatearNumero(totalPrecioFaltanteBoletasReservadas)}",
    10,
...
<template>
  ...
  <div id="all" :style="{ backgroundColor: colorFondo }">
    ...
    <div id="boletaDesplegableReserva" v-if="boletaReservada == true">
      ...
      <div>
        <button @click="verDatosParticipante">
    ...
    <div
      id="boletaDesplegableComprada"
      v-if="boletaComprada == true"
      :style="{ backgroundColor: boletaComprada }"
    >
      <button @click="BotonCerrar" class="cerrarBtn">x</button>
      <h3>
        Boleta
    ...
    <main>
      <section id="informacion">
        ...
        <div id="infoContenido">
          <p>
            🏆<span>{{
              mostrarBoletas == true ? formatearNumero(premio) : ""
            }}</span>
          </p>
          <p>
            💰<span>{{
              mostrarBoletas == true ? formatearNumero(precioBoleta) : ""
            }}</span>
          </p>
          <p>
      ...
      <section id="loteria">
        <button
          ...
          :key="i"
          @click="editarBalota(e, i)"
          :style="color(e.estado)"
        >
          {{ e.i < 10 ? "0" + e.i : e.i }}
        </button>
      </section>
      ...
      <section id="accion">
        ...
        <div id="accContenido">
          ...
          <button @click="mostrarPerzonalizar()">
      ...
      <section v-if="mostrarListadoBoletas" class="listado-boletas">
        ...
        <div class="papaCard">
          <div
            class="card"
            v-for="(cliente, index) in agruparBoletasCompradas()"
            :key="index"
          >
            <p>Nombre</p>
      ...
      <section id="formularioBD" v-if="mostrarRegistroDatosDeBoleta">
        ...
        <button
          class="adquirir"
          @click="adquirir()"
          v-if="actualizarB == false"
        >
          ADQUIRIR
        </button>
        <button class="adquirir" @click="actualizarpartipante()" v-else>
          actualizar
        </button>
      </section>
